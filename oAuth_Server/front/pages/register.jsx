import { Box, Flex, Text, Input, FormControl, FormLabel, useDisclosure, FormHelperText, Select, Radio, RadioGroup, Stack, useEditable, Checkbox, Center } from '@chakra-ui/react';
...
const register = () => {
  ...
  const verifyAccount = async () => {
    ...
    if (verifier == verifyNum) {
      try {
        const regiEmail = email + domain;
        const response = await axios.post(`${backend}/Oauth/user/oauthregister`, {
          email: regiEmail,
          password,
          gender,
          name,
          ...
          {sentEmail == false ? (
            ...
            <>
              <Box h="40rem">
                ...
                {loading ? <Input type="submit" onClick={verifyAccount} style={{ color: 'white' }} mt="2%" /> : <LoadingModal isOpen={isOpen} onClose={onClose} />}
              </Box>
            </>
          )}
        </FormControl>
