...
const AppInfo = () => {
	...
	const modifyRed = async () => {
		...
		const response = await axios.post(
			`${backend}/api/oauth/updateRedirect`,
			{ uri, email: email, appName: appName }
		);
	...
	const changeReq = async (k) => {
		...
		const response = await axios.post(
			`${backend}/api/oauth/getInfoUpdate`,
			{
	...
	const urireset = (e) => {
		...
		if (
			e.target.id !== "redirect" &&
			e.target.id !== "redirect1" &&
			e.target.id !== "redirect2" &&
			e.target.id !== "redirect3"
		) {
			setIsModifying(null);
		}
	};
		...
		<>
			<Box pt="5%" w="70%" mx="auto" my="0" onClick={urireset}>
				<Flex flexDirection={"column"} alignItems="center" mb="3%">
					...
					<Box fontSize={"120%"}>
						{" "}
						Application : {router.query.appName}{" "}
					</Box>
				</Flex>

				<Flex flexDirection={"column"} alignItems={"center"}>
					{showInfo == false ? (
						<Button onClick={revealInfo}>
							Rest api, client_secret 보기
						</Button>
					) : (
						<>
							<Box
